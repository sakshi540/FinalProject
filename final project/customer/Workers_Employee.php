...
// start bid users area
if(isset($_REQUEST['rBid'])){
  $tName = $_REQUEST['tName'];
  $id = $_REQUEST['id'];
  $bName = $_REQUEST['bName'];
  $bJobE = $_REQUEST['bJobE'];
  $bJobO = $_REQUEST['bJobO'];
...
// echo $bPhone."<br>";
// echo $bEmail."<br>";
  $sql = "INSERT INTO ".$tName."(`b_name`, `b_jobE`, `b_jobO`, `b_address`, `b_salaryE`, `b_phone`, `b_email`) 
  VALUES('$bName','$bJobE','$bJobO','$bAddress','$bSalary','$bPhone','$bEmail')";
  if(mysqli_query($conn,$sql)){
    $sql = "INSERT INTO bid_users(`b_name`, `b_jobE`, `b_jobO`, `b_address`, `b_salaryE`, `b_phone`, `b_email`) 
    VALUES('$bName','$bJobE','$bJobO','$bAddress','$bSalary','$bPhone','$bEmail')";
    if(mysqli_query($conn,$sql)){
      echo'<script> alert(" workers / employee bid successfully")</script>';
    }
  }else{
    echo'<script> alert(" unable to create table")</script>';
  }
}
// end bid users area
?>
...
<li class="nav-item ">
<a class="nav-link" href=""><b><i class="fa fa-home" aria-hidden="true"></i>Home</b> <span class="sr-only">(current)</span></a>
</li>
<li class="nav-item">
...
<!-- start users employees details  -->
  ...
  <h1 class="text-center" style="color:#D6098B"><b>All Workers and Employee's (details):</b></h1>
        ...
        $sql = "select * from users";
          ...
          if(mysqli_num_rows($result) > 0){
                  while($row = mysqli_fetch_assoc($result)){
                          $id = $row["id"] ;
                          $name = $row["db_name"] ;
                          $jobE = $row["db_jobExperience"] ;
                          $jobO = $row["db_jobOpted"];
                          $address = $row["db_address"] ;
                          $gender = $row["db_gender"] ;
                          $dob = $row["db_dob"] ;
                          $salary =  $row["db_salary"] ;
                          $phone =  $row["db_phone"] ;
                          $email= $row["db_email"] ;
                          $pass = $row["db_pass"] ;
                        ...
                        echo'<div class="card shadow p-4">';
                          echo'<div class="card-body">';
                            ...
                            if($conn){
                                ...
                                $sql = 'SELECT b_email FROM bid_users WHERE b_email ="'.$row["db_email"].'"';
                                if(mysqli_num_rows(mysqli_query($conn,$sql)) == 1){
                                  echo'<input type="submit" class="btn ntn-sm btn-outline-success mt-3" 
                                  value="Already Bid">';
                                }else{
                                  echo'<form action="" method="post">
                                  <input type="hidden" name="tName" value=' .$rName . '>
                                  <input type="hidden" name="id" value=' . $row['id'] . '>
                                  <input type="hidden" name="bName" value=' . $row['db_name'] . '>
                                  <input type="hidden" name="bJobE" value=' . $row['db_jobExperience'] . '>
                                  <input type="hidden" name="bJobO" value=' . $row['db_jobOpted'] . '>
                                  <input type="hidden" name="bAddress" value=' . $row['db_address'] . '>
                                  <input type="hidden" name="bSalary" value=' . $row['db_salary'] . '>
                                  <input type="hidden" name="bPhone" value=' . $row['db_phone'] . '>
                                  <input type="hidden" name="bEmail" value=' . $row['db_email'] . '>
                                  <input type="submit" class="btn ntn-sm btn-outline-primary mt-3" 
                                  name="rBid" value="Bid Now"></form>';
